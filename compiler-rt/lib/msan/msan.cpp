...
void PrintWarning(uptr pc, uptr bp) {
  Report("[msan.cpp] PrintWarning\n");
  PrintWarningWithOrigin(pc, bp, __msan_origin_tls);
}

void PrintWarningWithOrigin(uptr pc, uptr bp, u32 origin) {
  Report("[msan.cpp] PrintWarningWithOrigin\n");
  if (msan_expect_umr) {
    // Printf("Expected UMR\n");
...
void PrintWarningOOBWithOrigin(uptr pc, uptr bp, u32 origin) {
  Report("[msan.cpp] PrintWarningOOBWithOrigin\n");
  if (msan_expect_umr) {
    // Printf("Expected UMR\n");
    __msan_origin_tls = origin;
    msan_expected_umr_found = 1;
...
void UnpoisonParam(uptr n) {
  ...
  // internal_memset(__msan_param_tls, 0xfa, n * sizeof(*__msan_param_tls));
}
...
void ScopedThreadLocalStateBackup::Restore() {
  ...
  // [syx]
  Report("[msan.cpp] ScopedThreadLocalStateBackup::Restore\n");
  internal_memset(__msan_param_tls, 0xfa, sizeof(__msan_param_tls));
  internal_memset(__msan_retval_tls, 0xfa, sizeof(__msan_retval_tls));
  internal_memset(__msan_va_arg_tls, 0xfa, sizeof(__msan_va_arg_tls));
...
#define MSAN_MAYBE_WARNING(type, size)              \
  void __msan_maybe_warning_##size(type s, u32 o) { \
    ...
    (void)sp;                                       \
    if (UNLIKELY(s)) {                              \
      Report("[msan.cpp] __msan_maybe_warning_\n"); \
...
void __msan_warning_with_origin(u32 origin) {
  GET_CALLER_PC_BP_SP;
  (void)sp;
  Report("[msan.cpp] __msan_warning_with_origin\n");
...
void __msan_warning_oob_with_origin(u32 origin) {
  GET_CALLER_PC_BP_SP;
  (void)sp;
  Report("[msan.cpp] __msan_warning_oob_with_origin\n");
  PrintWarningOOBWithOrigin(pc, bp, origin);
  if (__msan::flags()->halt_on_error) {
    if (__msan::flags()->print_stats)
      ReportStats();
    Printf("Exiting\n");
...
void __msan_warning_with_origin_noreturn(u32 origin) {
  GET_CALLER_PC_BP_SP;
  (void)sp;
  Report("[msan.cpp] __msan_warning_with_origin_noreturn\n");
  PrintWarningWithOrigin(pc, bp, origin);
  if (__msan::flags()->print_stats)
    ReportStats();
  Printf("Exiting\n");
...
//[syx] -1 means shadow value is 0(initialized)
sptr __msan_test_shadow(const void *x, uptr size) {
  // Report("[msan.cpp] __msan_test_shadow: x=%p, size=%zu\n", x, size);
  unsigned char *s = (unsigned char *)MEM_TO_SHADOW((uptr)x);
  // for (uptr i = 0; i < size; ++i) Report("s[%zu] = %d\n", i, s[i]);
  if (__sanitizer::mem_is_zero((const char *)s, size)) {
    // Report("return -1\n");
    return -1;
  }

  // else if (mem_is_not_zero_or_one()){
  //   //report oob
  // }
  ...
  for (uptr i = 0; i < size; ++i)
    if (s[i]) {
      Report("[msan.cpp] __msan_test_shadow: return %zu\n", i);
      return i;
    }
  return -1;
}

//[syx] to-do: add the check for redzone value(0xFa et.al.),
//  and report the error.
void __msan_check_mem_is_initialized(const void *x, uptr size) {
  Report("[msan.cpp] __msan_check_mem_is_initialized: x=%p, size=%zu\n", x,
         size);
  ...
  // to-do: RRZ means read redzone memory
  // ReportRRMInsideAddressRange(__func__, x, size, offset);
...
//[syx] explain __msan_chain_origin:
// 1. get the caller's pc, bp, sp
// 2. get the store stack trace pc, bp
// 3. call ChainOrigin to get the chained origin
// 4. return the chained origin
// summary: get the chained origin of the current origin
...
// [syx] __sanitizer_unaligned_loadxx don't be involved
...
//[syx] __msan_finish_switch_fiber don't be involved
void __msan_finish_switch_fiber(const void **bottom_old, uptr *size_old) {
  ...
  // internal_memset(__msan_va_arg_tls, 0, sizeof(__msan_va_arg_tls));
  // [syx]
  Report("[msan.cpp] __msan_finish_switch_fiber\n");
  internal_memset(__msan_param_tls, 0xfa, sizeof(__msan_param_tls));
  internal_memset(__msan_retval_tls, 0xfa, sizeof(__msan_retval_tls));
  internal_memset(__msan_va_arg_tls, 0xfa, sizeof(__msan_va_arg_tls));
