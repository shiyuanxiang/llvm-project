...
void SetShadow(const void *ptr, uptr size, u8 value) {
  Report("[msan_poisoning.cpp] SetShadow: %p %zu %d\n", ptr, size, value);
  uptr PageSize = GetPageSizeCached();
  uptr shadow_beg = MEM_TO_SHADOW(ptr);
  uptr shadow_end = shadow_beg + size;

  // Report("[msan_poisoning] SetShadow: %p %zu %d\n", shadow_beg,
  //        shadow_end - shadow_beg, value);
  if (value ||
    ...
    // Report("[msan_poisoning] SetShadow page: %p %p %p %p\n", page_beg,
    // page_end,
...
// [syx] poison memory is to set the shadow bytes to 0xff
// invoked when allocating memory, the stack is for the origin
void PoisonMemory(const void *dst, uptr size, StackTrace *stack) {
  //[syx] set redzone
  SetShadow(dst, size, (u8)-1);
  // SetRedZone(dst,size,redzone_size);
  if (__msan_get_track_origins()) {
