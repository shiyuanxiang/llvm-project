...
void ReportOOB(StackTrace *stack, u32 origin) {
  ...
  Decorator d;
  Printf("%s", d.Warning());
  Report("WARNING: MemorySanitizer: out-of-boundary\n");
  Printf("%s", d.Default());
  stack->Print();
  if (origin) {
    DescribeOrigin(origin);
  }
  ReportErrorSummary("out-of-boundary", stack);
}
...
// [syx] to-do: RRZ means read redzone memory
void ReportRRZInsideAddressRange(const char *what, const void *start, uptr size,
                                 uptr offset) {
  Decorator d;
  Printf("%s", d.Warning());
  Printf("%sUninitialized bytes in %s%s%s at offset %zu inside [%p, %zu)%s\n",
